...
Archives.prototype.initialize = function initialize() {
  ...
  for (let vcm = 0; vcm < this.archives.length; vcm++) {
    ...
    let { host, port, protocol } = this.archives[vcm]
...
console.log("\n\nLOADING OUR TXS: "+number+" -- " + starting_point);
console.log(JSON.stringify(this.messages));

  for (let n = starting_point, m = starting_point; n < this.messages.length; n++) {
    //
...
console.log("WE HAVE A UNDEFINED TX ERROR: " + n);
console.log(JSON.stringify(this.messages));

      }
    } catch (err2) {

console.log("WE HAVE A DIFFICULT TX ERROR: ");
console.log(JSON.stringify(this.messages[n]));
