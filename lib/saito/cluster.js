...
console.log("pre stringify: " + new Date().getTime());
  for (let t = 0, f = 0, idx = 0; t < blk.transactions.length; t++, f++) {
    ...
    sigjson[f][idx]       = {}
    sigjson[f][idx].msg   = blk.transactions[t].returnSignatureSource();
    ...
    sigjson[f][idx].msig  = blk.transactions[t].transaction.msig;
    sigjson[f][idx].mhash = blk.transactions[t].transaction.mhash;
    sigjson[f][idx].txmsg = "";
    if (sigjson[f][idx].mhash != "") {
      sigjson[f][idx].txmsg  = JSON.stringify(blk.transactions[t].transaction.msg);
    }
  }
...
console.log("send to child " + f);
    this.child_forks[f].send({ validateTransactions: 1 , data : JSON.stringify(sigjson[f]) , child : f , max_children : this.child_forks.length });
  }
