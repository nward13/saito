...
console.log("pre stringify: " + new Date().getTime());
  for (let t = 0, f = 0, idx = 0; t < blk.transactions.length; t++, f++) {
    ...
    if (sigjson[f][idx].type >= 3) {
      sigjson[f][idx].txmsg  = this.fastSerialize(blk.transactions[t].transaction.msg);
    }
  }
console.log("pst stringify: " + new Date().getTime());
  ...
  for (let f = 0; f < this.child_forks.length; f++) {
    ...
    this.child_forks[f].send({ validateTransactions: 1 , data : this.fastSerialize(sigjson[f]) , child : f });
  }
...
Cluster.prototype.fastSerialize = function fastSerialize(obj) {
