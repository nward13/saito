...
Cluster.prototype.saveBlock = async function saveBlock(fp, blkjson) {

  if (this.enable_clustering == 0) {
    if (!fs.existsSync(fp)) {
      fs.writeFileSync(fp, JSON.stringify(blk.block, null, 4), 'UTF-8');
    }
    return 1;
  }
  ...
  if (this.saving_blks == 0) {
    this.saving_blks = 1;
  }

  if (this.blocks_to_save.length == 0) {
    this.child_forks[0].send({ saveBlock: 1 , filepath : fp , blk : blkjson });
  } else {
    this.blocks_to_save.push(blkjson);
    this.filepath.push(fp);
  }

  var promise = new Promise(function(resolve, reject) {
    setTimeout(() => {
      if (cluster_self.blocks_to_save.length < 1) {
	if (cluster_self.saving_blks == 0) {
          resolve(1);
	  return;
	}
      } else {
	  ...
	  // start next block
	  //
	  cluster_self.saving_blks = 1;
          this.child_forks[0].send({ saveBlock: 1 , filepath : cluster_self.filepath.shift() , blk : cluster_self.blocks_to_save.shift() });

        }
      }
    }, cluster_self.saving_blks_timer);
  });
