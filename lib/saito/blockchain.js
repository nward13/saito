...
console.log(" .... at start:    " + new Date().getTime());
...
console.log("About to Send Request for Missing Block: ");
  ...
  //
	...
	//
	// we save for now, and should add heuristics to avoid being flooded with bad blocks
	// in the next version of Saito -- tracking which blocks are disconnected and using
	// this to manage which chain we are on.
	//
	if (shared_ancestor_pos == 0 && lchain_prevhash != nchain_prevhash) {
	  console.log("HERE 1: " + shared_ancestor_pos + " -- " + this.blocks.length);
  ...
  if (i_am_the_longest_chain == 1 && this.index.hash.length > 1) {
    ...
    //
console.log(" .... into wind:   " + new Date().getTime());
...
console.log(" .... into succ:   " + new Date().getTime());
...
console.log(" .... start suc:   " + new Date().getTime());
  //
  // update newblock
  //
  ...
  // the wallet update.
  //
console.log(" .... pre saveb:   " + new Date().getTime());
  await this.app.storage.saveBlock(newblock, i_am_the_longest_chain);
console.log(" .... post saveb:  " + new Date().getTime());
  await this.saveBlockchain();
console.log(" .... post savec:  " + new Date().getTime());

console.log(" .... pre wallt:   " + new Date().getTime());
  ...
  if (i_am_the_longest_chain == 1) {
    ...
    //
    // propagateBlock when we know that it's saved to file
    //
...
console.log(" .... post walt:   " + new Date().getTime());
...
console.log(" .... blocksync:   " + new Date().getTime());
  //
  // cleanup
  //
  this.app.mempool.removeBlockAndTransactions(newblock);

console.log(" .... pre callbks: " + new Date().getTime());
  //
  ...
  // module callback
  //
console.log(" .... onNewblk:    " + new Date().getTime());
