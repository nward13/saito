...
console.log("validate block & queue:  " + (new Date().getTime()));
...
//  console.log("\n\n\nSTART TIME: "+startTime);
  ...
  console.log(JSON.stringify(blockchain_self.app.options.blockchain, null, 4));
...
console.log("LC pos: " + this.longestChain);
console.log(block_id + " ----> " + this.index.block_id[this.longestChain]);
...
console.log("triggering the longest chain re-write against here!");
  ...
  console.log(" ... to validate lcn:    "+ (new Date().getTime()));
  ...
  console.log(" ... am i the lc: " + i_am_the_longest_chain);
...
console.log(" ... chain rewrite: shared hash: " + shared_ancestor_hash);
...
console.log("VALIDATE INPUTS: " + blockchain_self.app.storage.validateBlockInputs(thisnewblk, null, 1));
...
console.log("UPDATING WALLET FOR: " + thisnewblk.returnHash());
...
console.log("LC pos: " + this.longestChain);
          ...
          for (let d = 1; d <= pos; d++) {
console.log(" continuing with d -> " + d + "     less than " + pos);
	    ...
	    if (blockchain_self.app.storage.validateBlockInputs(thisnewblk, null, 1) == 1) {
              blockchain_self.app.storage.spendBlockInputs(thisnewblk);
              blockchain_self.app.storage.onChainReorganization(thisnewblk.block.id, thisnewblk.returnHash(), 1);
console.log("UPDATING WALLET2 FOR: " + thisnewblk.returnHash());
              blockchain_self.app.wallet.onChainReorganization(thisnewblk.block.id, thisnewblk.returnHash(), 1);
              blockchain_self.app.modules.onChainReorganization(thisnewblk.block.id, thisnewblk.returnHash(), 1);
              this.index.lc[d] = 1;
              this.lc_hashmap[thisnewblk.returnHash()] = 1;
...
console.log("DONE ALL OF OUR ONCHAIN REORGS");
...
console.log("HERE WE ARE 2222");
    ...
    //
    console.log("validate longest chain in storage: "+i_am_the_longest_chain);
    // LOG INFO
...
console.log("REORGing WALLET: " + newblock.returnHash());
  ...
  for (let ti = 0; ti < newblock.transactions.length; ti++) {
    ...
    if (tx.isFrom(blockchain_self.app.wallet.returnPublicKey()) || tx.isTo(blockchain_self.app.wallet.returnPublicKey())) {
      updated_wallet = 1;
console.log("SENDING TX TO WALLET: "+i_am_the_longest_chain);
...
console.log("UPDATE BALANCE in MODS");
