...
Blockchain.prototype.addBlockToBlockchainSuccess = function addBlockToBlockchainSuccess(newblock, pos, i_am_the_longest_chain, forceAdd) {
  ...
  //
  // we do not delete the JSON copies here but 
  // when we remove from teh mempool. this allows
  // us to fetch the mempool copies right up until
  // the point we delete for fetchBlock, as we might
  // be propagating a big block before we have finished
  // validating and processing it.
  // 
  // see server.js for fetchBlock
  //
  //blockchain_self.blocks[pos].block.transactions = [];
