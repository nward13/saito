...
//console.log("RANDOM LOG");
  let sig_src = this.returnSignatureSource(app);
  console.log("SIG SRC", sig_src);
  if (!app.crypto.verifyMessage(sig_src, this.transaction.sig, this.returnSender())) {
...
Transaction.prototype.generateRebroadcastTransaction = function generateRebroadcastTransaction(tid, slip_id, avg_fee=2) {
  ...
  if (this.transaction.type >= 0 && this.transaction.type <= 3) {
    ...
    if (this.transaction.msg.tx != undefined) {
      ...
      newtx.transaction.msg.tx = this.stringify();
    }
...
Transaction.prototype.stringifySlipsTo = function stringifySlipsTo(escape_quotes=0) {
  ...
  if (escape_quotes == 0) {
    for (let i = 0; i < this.transaction.to.length; i++) {
      ...
      // TODO
      //
      // note that stringifySlipsTo is handled distinctly
      // when sending the request into the slip stringify
      // function as we want to reset the BID, TID and BHASH
      // values in this one particular case.
      //
      // this is a conceptually messy use of our stringify
      // function. it is needed to avoid breaking the sig
      // checks on slaves which are syncing from masters.
      //
      json      += this.transaction.to[i].stringify(2);
    }
  } else {
    for (let i = 0; i < this.transaction.to.length; i++) {
      if (i != 0) { json += ','; }
      json      += this.transaction.to[i].stringify(1);
    }
  }
  return json;
