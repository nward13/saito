...
Transaction.prototype.generateRebroadcastTransaction = function generateRebroadcastTransaction() {

  var fee = 2.0;
  var amt = this.transaction.from[0].amt-fee;
  if (amt < 0) { return null; }

  var newtx = new saito.transaction();

  newtx.transaction.sig = this.transaction.sig;
  newtx.transaction.from.push(new saito.slip(); 

  // reset message field
  newtx.transaction.msg = {};
  newtx.transaction.msg.rebroadcast = 1;

  // track how many times we loop
  newtx.transaction.msg.loop = 0;
  if (this.transaction.msg.loop > 0) { 
    newtx.transaction.msg.loop = this.transaction.msg.loop+1;
  }

  // original tx needed for validation
  if (this.transaction.msg.tx != "") {
    newtx.transaction.msg.tx = this.transaction.msg.tx;
  } else {
    newtx.transaction.msg.tx = this.transaction;
  }


  // create TO and FROM slips
  var from = new saito.slip(this.transaction.from[0].add, this.transaction.from[0].amt);
  var to   = new saito.slip(this.transaction.from[0].add, amt);

  newtx.transaction.from.push(to);
...
Transaction.prototype.isAutomaticallyRebroadcast = function isAutomaticallyRebroadcast(deadblk, newblk) {
  if (this.transaction.amt > 10) { return 1; }
  return 0;
}
