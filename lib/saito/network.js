...
Network.prototype.addPeer = function addPeer(peerjson, sendblks=1, sendtx=1, sendgtix=1) {
  ...
  for (let i = 0; i < this.peers.length; i++) {
    if (this.peers[i].peer.host == peerhost && this.peers[i].peer.port == peerport) {
      let config_changed = 0;
this.peers.splice(i, 1);
      if (sendblks == 1 && this.peers[i].sendblocks == 0) { 
	this.peers[i].sendblocks = 1;
	config_changed = 1;
      }
      if (sendtx   == 1 && this.peers[i].sendblocks == 0) {
	this.peers[i].sendtransactions = 1; 
	config_changed = 1;
      }
      if (sendgtix == 1 && this.peers[i].sendtickets == 0) {
	this.peers[i].sendtickets = 1;
	config_changed = 1;
      }

      //
      // may be reconnection attempt
      if (config_changed == 0) {
	this.peers.splice(i, 1);
	i--;
      }
