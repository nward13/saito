...
function Peer(app) {
  ...
  // tracking connection
  this.verified              = 0;		// set to 1 when they verify their publickey
  ...
  this.challenge_remote      = null;		// challenge remote creates
...
// debug print to info
//let message_starttime = new Date().getTime();
//peer_self.app.logger.logInfo("MESSAGE RECEIVED: "+message_starttime);
//peer_self.app.logger.logInfo(JSON.stringify(message, null, 2));
      ...
      if (message.request == "connect") {
        ...
        peer_self.challenge_remote  = message.data.challenge;

        var sigmessage = {};
        sigmessage.request               = "connect-sig";
        sigmessage.data = {};
        sigmessage.data.sig = peer_self.app.crypt.signMessage("_"+peer_self.challenge_remote, peer_self.app.wallet.returnPrivateKey());
        peer_self.socket.emit('request', JSON.stringify(sigmessage));
	...
	// update blockchain sync data
	  ...
	  if (peer_self.app.options.blockchain != null) {
	    if (peer_last_bid > peer_self.app.options.blockchain.target_block_id) {
	      peer_self.app.options.blockchain.target_block_id = peer_last_bid;
	      peer_self.app.modules.updateBlockchainSync(my_last_bid, peer_self.app.options.blockchain.target_block_id);
	    } else {
              peer_self.app.options.blockchain.target_block_id = my_last_bid;
    ...
    if (message.request == "connect-reply") {
      ...
      if (their_last_block_id > my_last_bid) {
        if (peer_self.app.options.blockchain != null) {
          if (their_last_block_id > peer_self.app.options.blockchain.target_block_id || peer_self.app.options.blockchain.target_block_id == undefined) {
            peer_self.app.options.blockchain.target_block_id = their_last_block_id;
            peer_self.app.modules.updateBlockchainSync(my_last_bid, peer_self.app.options.blockchain.target_block_id);
          } else {
            peer_self.app.options.blockchain.target_block_id = my_last_bid;
            peer_self.app.modules.updateBlockchainSync(my_last_bid, my_last_bid);
      ...
      if (message.request == "connect-sig") {
        ...
        if (sig != "") {
          ...
          if (peer_self.app.crypt.verifyMessage("_"+peer_self.challenge_local, sig, peer_self.peer.publickey) == 0) {
//console.log("CANNOT VERIFY SERVER is who it claims to be... "+peer_self.peer.publickey);
	  } else {
//console.log("JUST VERIFIED THAT " + peer_self.peer.publickey + " has signed this message....");
	    peer_self.verified = 1;
	  }
        }
      }
        ...
        if (tmpblk == null) { return; }
          ...
          if (peer_self.app.BROWSER == 0 && peer_self.app.SPVMODE == 0) {
            ...
            peer_self.app.blockchain.returnBlockByHash(t.hash, function(blk) {
              ...
              if (blk == null) {
                peer_self.app.logger.logInfo(`NULL BLK RETURNED BY returnBlockByHash at: ${Date.now()}`)
                return;
              }
...
Peer.prototype.returnConnectMessage = function returnConnectMessage() {
  ...
  message.data.challenge        = this.challenge_local;
...
//this.app.logger.logInfo("send request 1 to " + this.peer.host + " " + JSON.stringify(userMessage));
...
//this.app.logger.logInfo("send request 1-2 to " + this.peer.host + " " + JSON.stringify(userMessage));
...
this.app.logger.logInfo("send request 2-1 to " + this.peer.publickey + " " + JSON.stringify(userMessage));
...
this.app.logger.logInfo("send request 2-2 to " + this.peer.publickey + " " + JSON.stringify(userMessage));
...
//this.app.logger.logInfo("updating peer reliability to " + value + " for peer " + this.peer.publickey + " and value " + feature);
