...
Peer.prototype.addSocketEvents = function addSocketEvents() {
  ...
  try {
    ...
    this.socket.on('connect', function(){
      this.initializing = 0;
      peer_self.app.logger.logInfo(`client connect`)
    ...
    this.socket.on('request', function (data, mycallback=null) {
      ...
      if (message.request == "connect") {
        ...
        peer_self.app.logger.logInfo(`PEER CONNECTION INFO: ${JSON.stringify(message.data)}`)
        ...
        peer_self.app.logger.logInfo(`connect-sig msg sent: ${sigmessage.data.sig}`)
        console.log("connect-sig msg sent: " + sigmessage.data.sig);
        ...
        peer_self.app.logger.logInfo(`SETTING PEER TO: `)
        peer_self.app.logger.logInfo(JSON.stringify(peer_self.peer))
        ...
        peer_self.app.logger.logInfo(`PEER_LAST_BID called in peer class: ${peer_last_bid}`)
        peer_self.app.logger.logInfo(`BID_FROM_FID called in peer class: ${bid_from_fid}`)
        ...
        ////////////////
          ...
          // sync from scratch
          //
          peer_self.app.logger.logInfo(`TREATED LIKE FULL NODE -- PEER IN QUESTION: ${peer_self.peer.synctype}`)
          if (peer_last_bid === "" || peer_last_bid == 0) {
...
console.log("CONNECT REPLY MSG");

      peer_self.peer.publickey   = message.data.publickey;
      peer_self.challenge_remote = message.data.challenge;

      console.log("Handling the connect reply!");
...
peer_self.app.logger.logInfo(`CANNOT VERIFY SERVER is who it claims to be... ${peer_self.peer.publickey}`)
	
	  } else {
console.log("JUST VERIFIED THAT " + peer_self.peer.publickey + " has signed this message....");
          peer_self.app.logger.logInfo(`JUST VERIFIED THAT ${peer_self.peer.publickey} has signed this message...`)
      ...
      if (message.request == "missing block") {
        ...
        peer_self.app.logger.logInfo(`Message data from missing block: ${message.data}`)
...
Peer.prototype.fetchBlockchain = function fetchBlockchain() {
  this.app.logger.logInfo("fetchBlockchain IS CALLED")
...
Peer.prototype.sendBlockchain = function sendBlockchain(start_bid, type="full") {
  ...
  this.app.logger.logInfo(`SENDING BLOCKS FROM DATABASE STARTING WITH BLOCK: ${start_bid} and type ${type}`)
