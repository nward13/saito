...
//hyp - updates to make burn fee reduce hyperbolically
Block.prototype.calculateBurnFee = function calculateBurnFee(starting_burn_fee, starting_fee_step) {
  ...
  //
  // } }
  console.log("hyp: bf before: "+bf[0]+"--------------------------------------------------");
  //don't let the burn fee fall below one.
  if ( bf[0] >= 1 || target_time >= block_time ) {
    bf[0] = bf[0]*(target_time/block_time);
  }
  console.log("hyp: adjust bf by: "+target_time/block_time+"--------------------------------------------------");
  console.log("hyp: bf after: "+bf[0]+"--------------------------------------------------");
...
Block.prototype.returnTransactionFeesNeeded = function returnTransactionFeesNeeded(ts_start, ts_issue, ts_burn_fee, ts_fee_step) {
  ...
  var fees_needed = 0;
  //var fees_needed = Big(ts_burn_fee).minus(Big(ts_fee_step).times(Big(milliseconds_since_block)));
  // the big change to a hyperbolic curve
  // allow anyone to just make a block after twice the hearbeat period
  if (milliseconds_since_block <= (this.app.blockchain.heartbeat*2000)) {
    fees_needed = Big(ts_burn_fee/(milliseconds_since_block/1000));
  } else {
    fees_needed = Big(0);
  }
  console.log("hyp: feesneeded: "+fees_needed)+"----------------------------------------------------";

  return fees_needed.toFixed(8);
...
Block.prototype.validateTransactionFeesAdequate = function validateTransactionFeesAdequate() {
  ...
  console.log("hyp: block.id: "+this.block.id+"---------------------------------------------------------------");
  console.log("hyp: transaction fees needed during validation: "+ transaction_fees_needed);
  console.log("hyp: useable fees during validation: "+ usable_transaction_fees.plus(0.0));
  //not sure why this fails specifically for block 2 - i might need to read mempool.js
  if (this.block.id >= 3) {
    if (transaction_fees_needed.gt(usable_transaction_fees.plus(0))) { return 0; }
  }
