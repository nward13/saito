...
console.log("block to bundle block:   " + (new Date().getTime()));
  ...
  if (prevblock != null) {
    ...
    var txfees_surplus = Big(prevblock.returnTransactionFeesSurplus());
console.log("HOW MANY FEES TO DISTRIBUTE: " + txfees_surplus.toFixed(8));
    var total_revenue = txfees_surplus.plus(Big(prevblock.block.coinbase));
console.log("HOW MUCH COINBASE LUCRE: " + prevblock.block.coinbase);
console.log("HOW MUCH TOTAL REV: " + total_revenue.toFixed(8));
    var miner_share   = total_revenue.times(prevblock.block.paysplit).toFixed(8);
console.log("miner share: " + miner_share);
    var node_share    = total_revenue.minus(Big(miner_share)).toFixed(8);
console.log("node share: " + node_share);
    if (node_share    < 0)             { node_share = 0; }

console.log("ALL THE MONEY IN THE WORLD: " + prevblock.block.treasury + " || " + prevblock.block.reclaimed + " || " + miner_share + " || " + node_share);
...
console.log("RESULTING TREASURY: " + this.block.treasury);
...
console.log("set treasury to: " + this.block.treasury + "        and coinbase to: " + this.block.coinbase);
...
Block.prototype.returnTransactionFeesSurplus = function returnTransactionFeesSurplus() {
...
Block.prototype.validateTransactionFeesAdequate = function validateTransactionFeesAdequate() {
  ...
  var unixtime_start = this.app.blockchain.returnUnixtime(this.block.prevhash);
  var unixtime_current = this.block.unixtime;
  var transaction_fees_needed = Big(this.returnTransactionFeesNeeded(unixtime_start, unixtime_current, tb.returnBurnFee(), tb.returnFeeStep()));
