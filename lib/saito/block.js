...
console.log("block to bundle block:   " + (new Date().getTime()));
  ...
  if (prevblock != null) {
    ...
    var txfees_needed = Big(prevblock.returnTransactionFeesNeededForThisBlock());
    var total_revenue = txfees_needed.plus(Big(prevblock.block.coinbase));
    var miner_share   = total_revenue.times(prevblock.block.paysplit).toFixed(8);
    var node_share    = total_revenue.minus(miner_share).toFixed(8);
	...
	for (var i = 0; i < blk.transactions.length; i++) {
	  ...
	  for (var ii = 0; ii < blk.transactions[i].transaction.to.length; ii++) {
	    ...
	    slip.bhash = blk.returnHash();
		  ...
		  console.log("THIS SLIP IS NOT SPENT: " + JSON.stringify(slip));

		  //////////////////////////////////////////
		  // automatic transaction rebroadcasting //
		  //////////////////////////////////////////	// deadblk, newblk
		  if (blk.transactions[i].isAutomaticallyRebroadcast(blk, block_self) == 1) {
		  console.log("ATR!");
		    current_rebroadcast++;
		    ...
		    newtx.transaction.from[0].bid = block_self.block.id;
...
Block.prototype.returnSurplusFees = function returnSurplusFees() {
  ...
  return transaction_fees.minus(transaction_fees_needed).toFixed(8);
...
Block.prototype.returnTransactionFeesUsable = function returnTransactionFeesUsable() {
  ...
  return parseFloat(total_fees.toFixed(8));
}
