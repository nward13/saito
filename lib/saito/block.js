...
function Block(app, blkjson="", confirmations=-1) {
  ...
  this.rawjson                = "";     // rawjson of block, saved to speed-up processing
...
console.log(" -- blk create 1: " + new Date().getTime());

  if (blkjson != "") {
    try {
console.log(" -- blk create 1-1: " + new Date().getTime());
      // this.block = JSON.parse(blkjson);

      // this is only a 3% efficiency save? how does UTF-8 encoding get handled
      // otherwise.
      // need this to handle when the servers send buffers to each other
     var blk_json = blkjson.toString("utf8")
     console.log(blk_json);
     this.block = JSON.parse(blk_json);
console.log(" -- blk create 1-2: " + new Date().getTime());
      for (var i = 0; i < this.block.txsjson.length; i++) {
        ...
        if (!this.transactions[i].is_valid) {
          this.is_valid = 0;
console.log("FAILURE TO IMPORT TX: " + JSON.stringify(this.block.txsjson[i]));
          return;
        }
      }
console.log(" -- blk create 1-3: " + new Date().getTime());
    } catch (err) {
...
console.log(" -- blk create 2: " + new Date().getTime());
...
console.log(" -- blk create 3: " + new Date().getTime());
...
console.log(" -- blk create 4: " + new Date().getTime());
...
console.log("VALIDATING REBROADCAST TXS");
  ...
  // otherwise, load the relevant blocks
  //
console.log(" ... opening block_id: " + eliminated_block_id + " -- " + new Date().getTime());
  var eblk = await this.app.storage.loadSingleBlockFromDiskById(eliminated_block_id);
console.log(" ... opened block_id: " + eliminated_block_id + " -- " + new Date().getTime());
  var gblk = await this.app.storage.loadSingleBlockFromDiskById(goldenticket_block_id);
console.log(" ... opened block_id: " + goldenticket_block_id + " -- " + new Date().getTime());
...
console.log(" .... validate A:  " + new Date().getTime());
...
console.log(" .... validate B:  " + new Date().getTime());
...
console.log(" .... validate C:  " + new Date().getTime());
...
console.log(" .... validate C2: " + new Date().getTime());
...
console.log(" .... validate D:  " + new Date().getTime());
  //
...
console.log(" .... validate E:  " + new Date().getTime());
...
console.log(" .... validate F:  " + new Date().getTime());
  //
...
console.log(" .... validate G:  " + new Date().getTime());
...
console.log(" .... validate H:  " + new Date().getTime());
  if (gttx_count > 1) {
...
console.log(" .... validate I:  " + new Date().getTime());
  // no golden ticket
  if (gttx == null) {
...
console.log(" .... validate J:  " + new Date().getTime());

  } else {
console.log(" .... validate K:  " + new Date().getTime());
...
console.log(" .... validate L:  " + new Date().getTime());
...
console.log(" .... validate M:  " + new Date().getTime());
...
console.log(" .... validate N:  " + new Date().getTime());
  return 1;
}
