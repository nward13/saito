...
Wallet.prototype.createFeeTransaction = function createFeeTransaction(my_fee) {
  ...
  var tslip = new saito.slip(this.returnPublicKey(), my_fee.toFixed(8), 0);
  tx.transaction.to.push(tslip);
...
console.log(value + " <------- " + this.wallet.utxi[i].amt);
...
console.log("NOT ENOUGH INPUTS FOR: " + amt);
  return null;
}
