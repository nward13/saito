  ...
  if (this.wallet.privatekey == "") {
    ...
    if (this.app.options.wallet != null) {
      ...
      if ( this.app.options.wallet.is_testing != undefined) {
        console.log("is testing", this.app.options.wallet.is_testing);
...
Wallet.prototype.signTransaction = function signTransaction(tx) {
  ...
  // ensure message hash
  ///
  tx.transaction.mhash  = this.app.crypto.hash(Buffer.from((JSON.stringify(tx.transaction.msg)+tx.transaction.ts), 'utf-8').toString('base64'));
...
console.log("regenerating hashmap: " + new Date().getTime());
    this.inputs_hmap = [];
    this.outputs_hmap = [];
    this.inputs_hmap_counter = 0;
    this.outputs_hmap_counter = 0;
...
console.log("done regener hashmap: " + new Date().getTime());
