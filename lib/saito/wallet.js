...
/////////////////////////////////////
// reorganizeTransactionsAtBlockId //
/////////////////////////////////////
//
// this function is triggered in one place -- whenever the blockchain 
// adds a block in AddBlockToBlockchainPartTwo and the longest_chain
// is 1.
//
// We do this because there is an edge case where a wallet that syncs 
// to a forked version of the final block can be convinced that both
// blocks are valid and part of the longest chain. So in these cases 
// we check and set any other txs at that BID to 0. 
//
// @params {integer} block_id
// @params {integer} am_i_the_longest_chain  (will be zero)
//
Wallet.prototype.reorganizeTransactionsAtBlockId = function reorganizeTransactionsAtBlockId(block_id, lc) {
  for (let m = this.wallet.utxi.length-1; m >= 0; m--) {
    if (this.wallet.utxi[m].bid == block_id) { 
      this.wallet.utxi[m].lc = lc; 
    }
    else {
      if (this.wallet.utxi[m].bid < block_id) { 
  ...
  for (let i = this.spent_slips_idx; i < this.wallet.utxi.length; i++) {
    if (this.wallet.utxi[i].lc == 1 && this.wallet.utxi[i].bid >= lowest_block) {
      if (this.spent_slips[i] == 0) {
        ...
        value = value.plus(Big(this.wallet.utxi[i].amt));
