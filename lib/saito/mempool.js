...
console.log("\n\n\nstart to bundle block:   " + (new Date().getTime()));
  ...
  //
  // now handled in block.js because rebroadcast txs
  // pay fees too!
  //
  let my_fees    = Big(nb.returnSurplusFees());
...
Mempool.prototype.fetchBlock = function fetchBlock(peer, block_hash) {
  ...
  mempool_self.app.logger.logInfo(`MD2 IN fetchBlock HERE: ${JSON.stringify(md2)}`)
    ...
    mempool_self.downloading_timer = setInterval(function() {
      ...
      try {
        if (mempool_self.block_size_current <= mempool_self.block_size_cap) {
          ...
          let { bhash, peer } = dlblk
          let { host, port, protocol } = peer.peer

          let dlblk_url = `${protocol}://${host}:${port}/blocks/${bhash}`

          mempool_self.app.logger.logInfo(`Peer conteact: ${dlblk.peer}`)
          mempool_self.app.logger.logInfo(`Download Block URL: ${dlblk_url}`)
          ...
          if (mempool_self.app.BROWSER == 0) {
            ...
            request(requestSettings, function(err, res, body) {
              if (err) {
                ...
                mempool_self.app.logger.logError(`Failed download of block from queue`, err);
              ...
              mempool_self.app.logger.logInfo(`Newblk downloaded from peer: ${newblk}`)
  ...
  for (let i = 0; i < this.transactions.length; i++) {
    if (paysplit_vote == -1) {
      if (this.transactions[i].transaction.ps <= 0) {
        v = v.plus(Big(this.transactions[i].returnFeeUsable()));
      }
    }
    if (paysplit_vote == 0) {
      v = v.plus(Big(this.transactions[i].returnFeeUsable()));
...
Mempool.prototype.tryToBundleBlock = function tryToBundleBlock() {
  ...
  if (mempool_self.app.monitor.readyToBundleBlock() == 1) {
    ...
    console.log((new Date()) + ": " + fees_needed.toFixed(8) + " ---- " + fees_available.toFixed(8) + " (" + mempool_self.transactions.length + "/"+mempool_self.returnNormalTransactionsInMempool()+" -- "+mempool_self.app.wallet.returnBalance()+")");
