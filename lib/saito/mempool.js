...
Mempool.prototype.addTransaction = function addTransaction(tx, relay_on_validate=1) {
  ...
  if (transaction_imported == 0) {
	...
	//
	  ...
	  // we could hypothetically produce a block, but still
...
console.log("\n\n\n\n");
console.log("DOWNLOADING: " + dlblk_url);
console.log("\n\n\n\n");
...
Mempool.prototype.tryToBundleBlock = function tryToBundleBlock() {
  ...
  if (mempool_self.app.monitor.readyToBundleBlock() == 1) {
    ...
    if (latestBlk != null) {
      ...
      console.log("hyp: milliseconds since last block and limit: "+milliseconds_since_block+" - "+this.app.blockchain.heartbeat*2000);
      if (milliseconds_since_block <= (this.app.blockchain.heartbeat*2000)) {
        fees_needed = Big(latestBlk.returnBurnFee()/(milliseconds_since_block/1000));
      } else {
        fees_needed = Big(0);
      }
