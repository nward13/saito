...
Mempool.prototype.removeBlock = function removeBlock(blk) {
  ...
  for (let b = this.blocks.length-1; b >= 0; b--) {
    if (this.blocks[b].returnHash() == blk.returnHash()) {
      ...
      //
      // NOW we delete the item from our mempool....
      // this will delete from the item in our callback
      // queue and help keep memory more compact. we 
      // delay deletion until now because we send fetchBlock
      // data from our mempool if possible.
      //
      // see server.js and blockchain.js for details
      //
      this.blocks[b].block.transactions = [];
