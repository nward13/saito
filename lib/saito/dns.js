...
DNS.prototype.initialize = function initialize() {
  ...
  //
  this.app.logger.logInfo(`DNS Initialized`)
  ...
  for (let i = 0; i < this.dns.domains.length; i++) {
    ...
    this.app.logger.logInfo(`DNS attempting to connect to the following peer: ${host}`)
    this.app.network.addPeer(host, port, protocol);
  }
