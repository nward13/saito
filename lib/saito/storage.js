...
Storage.prototype.deleteBlocks = function deleteBlocks(block_id) {
  ...
  storage_self.queryDatabaseArray(sql, params, function (err, rows) {
    ...
    if (err) {
      ...
      this.app.logger.logError("Error is: " + JSON.stringify(err.message));
    }
  });
...
console.log("PURGE SLIP: " + slip_map_index + " in block " + block_hash);
      ...
      fs.unlink(block_filename, function(err) {
        if (err) {
          this.app.logger.logError("Error thrown in purgeBlockStorage", {message:"", stack: err});
        }
      });
...
Storage.prototype.execDatabase = function execDatabase(sql, params, callback=null) {
  ...
  this.db.run(sql, params, function (err, row) {
    ...
    if (err) {
      ...
      storage_self.app.logger.logError("Error is: " + JSON.stringify(err.message));
    }
  });
}
...
Storage.prototype.onChainReorganization = function onChainReorganization(block_id, block_hash, lc) {
  ...
  this.db.run(sql, {
    ...
    if (err) {
      storage_self.app.logger.logError(`db.run in onChainReorganization failed: ${JSON.stringify(err)}`, {message: "", stack: ""})
      storage_self.app.logger.logError("Error is: " + JSON.stringify(err.message));
    }
  });
}
...
//
Storage.prototype.returnBlockFilenameByHash = function returnBlockFilenameByHash(block_hash, mycallback) {
...
Storage.prototype.queryDatabase   = function queryDatabase(sql, params, callback) {
  ...
  this.db.get(sql, params, function (err, row) {
    ...
    if (err) {
      storage_self.app.logger.logError("Error from queryDatabase", {message: "", stack: err})
    }
  });
}
...
Storage.prototype.queryDatabaseArray = function queryDatabaseArray(sql, params, callback) {
  ...
  this.db.all(sql, params, function (err, rows) {
    ...
    if (err) {
      storage_self.app.logger.logError("Error from queryDatabaseArray", {message: "", stack: err});
    }
  });
}
...
console.log("1. SAVE BLOCK MEANS INSERT HASHMAP SLIP!");
...
console.log("2. SAVE BLOCK MEANS INSERT HASHMAP SLIP!");
...
console.log(" INSERTING SLIP: " + slip_map_index);
...
console.log("UNEXPECTED: " + blk.block.transactions.length + " -- " + blk.transactions.length);
console.log(JSON.stringify(blk.transactions[0]));
console.log("AND TWO: " );
console.log(JSON.stringify(blk.block.transactions[0]));
...
console.log("\n\n\n\n\nERROR WAS REPORTED BEFORE THIS\n\n\n\n");
...
Storage.prototype.sendBlockchainChunk = function sendBlockchainChunk(peer) {
  ...
  this.app.storage.queryDatabaseArray(sql, params, function (err, rows) {
    if (err) {
      storage_self.app.logger.logError("error thrown in sendBlockchainChunk", {message: "", stack: err})
    }
...
console.log(" INSERTING SLIP SPENT: " + slip_map_index + " --> " + newblock.block.id)
...
console.log("UNSPEND SLIP: " + slip_map_index + " --> " + -1)
        shashmap.insert_slip(slip_map_index, -1);
      }
    }
  }
...
Storage.prototype.unwindChain = function unwindChain(newblock, pos, shared_ancestor_index_pos, new_block_idxs, new_block_hashes, new_block_ids, old_block_idxs, old_block_hashes, old_block_ids, i_am_the_longest_chain, forceAdd, current_unwind_index, resetting_flag) {
  ...
  console.log(" ... unwind old chain: " + old_block_hashes[current_unwind_index] + " ("+current_unwind_index+")");
  ...
  if (old_block_hashes.length > 0) {
	...
	// we should not be in this situation in normal operations, as we are
	// unwinding a block that is technically already part of the longest
	// chain. So something is wrong. But we want to try and handle this
	// gracefully so that we don't trigger any other errors.
	...
	// 
	// this needs to be removed, but keeping it around until we're sure
	// it isn't screwing something up and we don't need to edit it as
	// part of our fix.
	//
...
console.log("validate block inputs in storage validateBlockInputs() --> with prevblock permitted: " + prevblk_permitted_null);
    ...
    for (let bb = 0; bb < newblock.transactions[b].transaction.from.length; bb++) {
	  ...
	  if (newblock.transactions[b].transaction.rb > 0 || newblock.transactions[b].transaction.rb == -1 || newblock.transactions[b].transaction.rb == -2) {} else {
            ...
            if (newblock.transactions[b].transaction.from[bb].bid < storage_self.app.blockchain.blk_limit) {
              ...
              console.log("RETRIEVED VALUE: " + shashmap.slip_value(slip_map_index));
	      console.log("in block: " + newblock.returnHash() + " -- " + newblock.block.id);
...
Storage.prototype.validateLongestChain = function validateLongestChain(newblock, pos, shared_ancestor_index_pos, new_block_idxs, new_block_hashes, new_block_ids, old_block_idxs, old_block_hashes, old_block_ids, i_am_the_longest_chain, forceAdd) {
  ...
  console.log("validating longest chain: " + i_am_the_longest_chain);
...
console.log("UNLC : " + old_block_ids[x]);
...
console.log("UNLC : " + new_block_ids[x]);
  ...
  //
  if (old_block_hashes.length > 0) {
console.log("TESTING A: unwinding old chain");
...
console.log("TESTING B: winding new chain");
...
console.log("WINDING WHICH BLOCK: " + this_block_hash);
