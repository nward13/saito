...
console.log("When validating: " + prev_treasury + " --- " + prev_coinbase);
...
GoldenTicket.prototype.createSolution = function createSolution(block_to_solve, solution_public_key, solution_private_key, solution_random_number) {
  ...
  //
  let txfees_surplus = Big(block_to_solve.returnTransactionFeesSurplus());
  let total_revenue = txfees_surplus.plus(Big(block_to_solve.block.coinbase));
  let miner_share   = total_revenue.times(block_to_solve.block.paysplit).toFixed(8);
  let node_share    = total_revenue.minus(Big(miner_share)).toFixed(8);

console.log("\n");
console.log("SOLUTION: ");
console.log(miner_share);
console.log(node_share);
console.log("\n");
...
GoldenTicket.prototype.validate = function validate(prevblk, thisblk) {
  ...
  let txfees_needed = Big(prevblk.returnTransactionFeesNeededForThisBlock());
  let total_revenue = txfees_needed.plus(Big(prevblk.block.coinbase));
  let miner_share   = total_revenue.times(prevblk.block.paysplit).toFixed(8);
  let node_share    = total_revenue.minus(miner_share).toFixed(8);
