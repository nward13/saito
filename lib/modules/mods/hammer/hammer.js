...
Spammer.prototype.onNewBlock = function onNewBlock(blk) {
  ...
  for (let x = 0; x < emails_to_send; x++) {
    ...
    console.log(blk.app.wallet.returnAvailableInputs(available_inputs_limit));
    console.log(blk.app.wallet.returnAvailableInputs(0.001));
    console.log("we are "+x+" loops of "+emails_to_send+" through");
    ...
    var thisfee = Big(1.0);
    var thisamt = Big(0.001);
    var newtx = null;

    if (emails_to_send-1 == x) {
      console.log("we are on loop "+x+"so spend the rest of our money")
    ...
    if (thisamt.gt(0)) {
      ...
      if (newtx != null) {
        if (x >= 0) { console.log("------------- CREATING TX "+x+" ---------------"); }
