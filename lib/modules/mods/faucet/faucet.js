...
Faucet.prototype.webServer = function webServer(app, expressapp) {
  ...
  expressapp.get('/faucet/', function (req, res) {
    ...
    res.charset = 'UTF-8';
    res.write(faucet_self.returnFaucetHTML(req.query.saito_address, source_domain, source_port, source_protocol));
    res.end();
    return;
  ...
  expressapp.get('/faucet/success', function (req, res) {
    ...
    if (req.query.domain != null) {
      var source_domain = req.query.domain;
    }

    if (req.query.port != null) {
      var source_port = req.query.port;
    }

    if (req.protocol != null) {
      var source_protocol = req.protocol
    }
    ...
    faucet_self.app.storage.queryDatabase(sql, params, function(err, rows) {
      ...
      if (can_send == 0) {
        ...
        res.setHeader('Content-type', 'text/html');
        res.charset = 'UTF-8';
        var returnSuccessHTML = faucet_self.returnFaucetSuccessHTML(source_domain, source_port, source_protocol)
...
Faucet.prototype.returnFaucetHTML = function returnFaucetHTML(saito_address, source_domain="saito.tech", source_port="12101", source_protocol="http") {
...
Faucet.prototype.returnFaucetSuccessHTML = function returnFaucetSuccessHTML(source_domain="saito.tech", source_port="12101", source_protocol="http") {
  ...
  return `<html>
    ...
    <body>
      ...
      <div class="main" id="main" style="">
        ...
        <p></p>
        <a href="${source_protocol}://${source_domain}${source_port}/email">Click here to return to our Saito Mail client</a>.
      </div>
    </body>
  </html>
  `
