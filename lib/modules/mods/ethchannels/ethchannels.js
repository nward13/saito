...
// TODO:
// - fix ambiguity between Eth / Wei Conversions
// - check with very small amounts of Eth
// - check that emails persist and new tx's aren't repeated
// - add form checks. i.e. if payment amt !> current balance, don't let them send its
// *- add the format author thing and test it
  ...
  if (conf === 0) {
    ...
    if (tx.transaction.to[0].add === app.wallet.returnPublicKey()) {
      ...
      if (txmsg.request === ec_self.channelOpenedString) {
        ...
        if (ec_self.incomingExists(peerSaitoAddr)) {
          ...
          // TODO: send unrequested email tx from the other user's from confirmation
          return false;
        }
        ...
        return ec_self.getOpenBlock(
          ...
          // TODO: handle this with a notification to the channel sender from the 'from' confirmation
          if (!channelOpen) {
            return false;
          } else {
            ...
            ec_self.incomingChannels[channelId].deposit = web3.utils.toWei(txmsg.data.deposit.toString(), 'ether');
            ec_self.incomingChannels[channelId].peerBal = web3.utils.toWei(txmsg.data.deposit.toString(), 'ether');
      ...
      if (txmsg.request === ec_self.paymentSentString) {
        ...
        var amt = web3.utils.toWei(txmsg.data.amt.toString(), 'ether');
        var peerSaitoAddr = tx.transaction.from[0].add;
        ...
        console.log("paymentSent to us confirmation.");
        console.log("Amt: ", amt);
        console.log("My balance: ", ec_self.incomingChannels[channelId].myBal);
        if (!(amt >= ec_self.incomingChannels[channelId].myBal)) {
          // TODO: handle this within the form, should not let tx be sent
          // by channel sender
          console.log("About to return false from paymentSent to us confirmation");
          return false;
        }
        ...
        return ec_self.verifySig(
          ...
          // TODO: handle this with notification to channel sender from
          // 'from' confirmation
          if (!result) {
            return false;
          } else {
      ...
      if(txmsg.request === ec_self.channelClosedString) {
        ...
        return ec_self.isChannelClosed(channel.myEthAddr, channel.peerEthAddr, channel.openBlock)
          .then(result => {
            ...
            // TODO: handle this with notification to us
            if (!result) {
              console.log("The channel is not closed.");
              // TODO:
              // ec_self.channelCloseFailedEmail(tx, app);
              return false;
            } else {
    ...
    if (tx.transaction.from[0].add === app.wallet.returnPublicKey()) {
      ...
      if (txmsg.request === ec_self.channelOpenedString) {
        ...
        return ec_self.getOpenBlock(
          ...
          ec_self.isChannelOpen(
            ...
            if (!channelOpen) {
              // TODO:
              // ec_self.channelOpenFailedEmail(tx, app);
              return false;
            } else {

              // Now we know that channel has been opened, so we finish creating it in our module
              var channelId = ec_self.getOutgoingIndex(tx.transaction.to[0].add);
              ec_self.outgoingChannels[channelId].deposit = web3.utils.toWei(txmsg.data.deposit.toString(), 'ether');
              ec_self.outgoingChannels[channelId].myEthAddr = txmsg.data.senderEthAddr;
              ec_self.outgoingChannels[channelId].myBal = web3.utils.toWei(txmsg.data.deposit.toString(), 'ether');
      ...
      if (txmsg.request === ec_self.paymentSentString) {
        ...
        // NOTE: amt is always the recipient's new total balance, not 
        // the additional payment amount
        console.log("paymentSent from us confirmation.");
        console.log("Amt: ", amt);
        console.log("Peer balance: ", ec_self.outgoingChannels[channelId].peerBal);
        if (!(amt >= ec_self.outgoingChannels[channelId].peerBal)) {
          console.log("About to return false from paymentSent from us confirmation");
          return false;
        }
        ...
        return ec_self.verifySig(
          ...
          if (!result) {
            // TODO:
            // ec_self.invalidSigEmail(tx, app);
            return false;
          } else {
...
EthChannels.prototype.requestChannelEmail = function requestChannelEmail(tx, app) {
  ...
  var emailBody = '<div id="open_channel_body">'
    + '<p>You have recieved a request to open an Ethereum payment channel to '
...
EthChannels.prototype.recipientOpenConfirmEmail = function recipientOpenConfirmEmail(tx, app, channel) {
  ...
  //     +'<li>Sender\s current channel balance: ' 
  //       + web3.utils.fromWei(channel.peerBal, 'ether') + ' ETH ('
  //       + channel.peerBal + ' Wei)' + '</li>'
  //     +'<li>Your current channel balance: ' 
  //       + web3.utils.fromWei(channel.myBal, 'ether') + ' ETH ('
...
// ** Signature should be keccak256(channel_key, amt_to_pay_recipient)
  ...
  //       +'<li>Open Block: ' + channel.openBlock + '</li>'
  //       +'<li>Deposit Amount: ' + channel.deposit + '</li>'
  //       +'<li>Recipient\'s current channel balance: ' 
  //         + web3.utils.fromWei(channel.peerBal, 'ether') + ' ETH ('
  //         + channel.peerBal + ' Wei)' + '</li>'
  //       +'<li>Your current channel balance: ' 
  //         + web3.utils.fromWei(channel.myBal, 'ether') + ' ETH (' 
...
EthChannels.prototype.recipientPaymentConfirmEmail = function recipientPaymentConfirmEmail(tx, app, channel) {
  ...
  var emailBody = '<div class="recipient_payment_confirm">'
    ...
    +'<div id="close_channel_page_2" style="display:none">'
      ...
      +', to send the following transaction to the payment channels contract on the Rinkeby testnet:'
      +'<div><strong>closeChannel(' + channel.peerEthAddr + ', ' + channel.openBlock
      + ', ' + channel.myBal + ', ' + channel.lastSig + ')</strong></div>'
  ...
  //       +'<li>Open Block: ' + channel.openBlock + '</li>'
  //       +'<li>Deposit Amount: ' + channel.deposit + '</li>'
  //       +'<li>Sender\'s current channel balance: ' + web3.utils.fromWei(channel.peerBal, 'ether')
  //         + ' ETH (' + channel.peerBal + ' Wei)' + '</li>'
  //       +'<li>Your current channel balance: ' + web3.utils.fromWei(channel.myBal, 'ether')
  //         + ' ETH (' + channel.myBal + ' Wei)' + '</li>'
  //       +'<li>Most recent payment signature: ' + channel.lastSig + '</li>'
  //     +'</ul>'
  //     +'<p>You will recieve a confirmation email for every new payment '
  //     +'that you recieve.</p>'
...
// TODO: make sure that tx value is in wei for the openChannel 'tx to send'
...
EthChannels.prototype.channelInfoHtml = function channelInfoHtml(channel, toSender=true) {
  ...
  // var paymentSent;
  // if (toSender) {
  //   paymentSent = channel.peerBal === 0;
  // } else {
  //   paymentSent = channel.myBal === 0;
  // }
...
EthChannels.prototype.displayEmailForm = function displayEmailForm(app) {
  ...
  // TODO: Add a description of how to use the channels

  $('#module_editable_space').html(
    '<div id="module_instructions" class="module_instructions">'
      +'<h3>Welcome to the EthChannels Module</h3>'
      +'<p>To request a payment channel, add the requested channel sender to the '
...
ModTemplate.prototype.attachEmailEvents = function attachEmailEvents(app) {
  ...
  $('#open_channel').off().on('click', function() {
    ...
    $('#deposit').text(web3.utils.toWei(openData[1].value.toString(), 'ether') + ' Wei');
...
// ** Signature should be keccak256(channel_key, amt_to_pay_recipient)
  ...
  $('#close_tx_submitted').off().on('click', function() {
    ...
    // TODO: check for createUnsignedTransaction() returning null
    if (newtx === null) {
