...
Money.prototype.returnTableContents = function returnTableContents(app) {
  ...
  for (var b = mv, bind = 0; b >= ma; b--, bind++) {
    ...
    blockid[bind]    = b;
    treasury[bind]   = Big(tmpblk.returnTreasury()).minus(Big(tmpblk.returnCoinbase()));
    coinbase[bind]   = Big(tmpblk.returnCoinbase());
    reclaimed[bind]  = Big(tmpblk.returnReclaimed());
    spent[bind]      = Big(0.0);
    unspent[bind]    = Big(0.0);
    fees[bind]       = Big(tmpblk.returnTransactionFeesTotal());
    gt[bind]         = 0;

/*****
console.log("OG Treas: "  + tmpblk.returnTreasury());
console.log("OG Coinb: "  + tmpblk.returnCoinbase());

console.log("Treasury: "  + treasury[bind].toFixed(9));
console.log("Coinbase: "  + coinbase[bind].toFixed(9));
console.log("Reclaimed: " + reclaimed[bind].toFixed(9));
console.log("Spent: "     + spent[bind].toFixed(9));
console.log("Unspent: "   + unspent[bind].toFixed(9));
console.log("Fees: "      + fees[bind].toFixed(9));
*****/
    ...
    if (tmpblk != null) {
	  ...
	  if (slip1_spent == 1) {
	    ...
	    unspent[bind] = unspent[bind].plus(Big(parseFloat(slip1.amt)));
	  }
        }
      }
    }
  ...
  for (var sd = 0; sd < spent.length; sd++) {
    ...
    if (gt[sd] == 1) { if (coinbase[sd+1] != null) { total_coinbase = total_coinbase.minus(coinbase[sd+1]); } }
