...
Init.prototype.onNewBlock = function onNewBlock(blk) {
  ...
  // golden chunk //
  //////////////////
  if (blk.block.id == this.max_block) {

    newtx                  = new saito.transaction();
    newtx.transaction.from = blk.app.wallet.returnAdequateInputs(100001);
    newtx.transaction.to.push(new saito.slip(newtx.atr_trapdoor_address, 90, 1));
    newtx.transaction.to[0].type = 5;
    newtx.transaction.ts   = new Date().getTime();
    newtx.transaction.type = 5; // golden chunk
    newtx                  = this.app.wallet.signTransaction(newtx);
