...
console.log("ON NEW BLOCK IN SPAMMER: ");
...
console.log("ON NEW BLOCK IN SPAMMER 2: ");

  for (let x = 0; x < emails_to_send; x++) {

console.log("ON NEW BLOCK IN SPAMMER 2: ");
    var available_inputs_limit = 0.5;
console.log("ON NEW BLOCK IN SPAMMER 21: ");
    var available_inputs       = Big(blk.app.wallet.returnAvailableInputs(available_inputs_limit));
console.log("ON NEW BLOCK IN SPAMMER 22: ");

    if (available_inputs.gt(available_inputs_limit) || (x > 0 && x >= emails_to_send-1)) { 
console.log("CREATED TXS: "+x);
      return; 
    }
console.log("ON NEW BLOCK IN SPAMMER 23: ");

    var thisfee = Big(2.0);
console.log("ON NEW BLOCK IN SPAMMER 24: ");
    var thisamt = Big(0.0);
console.log("ON NEW BLOCK IN SPAMMER 25: ");
    var newtx = null;

console.log("ON NEW BLOCK IN SPAMMER 3: ");
    if (emails_to_send == 1) {
      thisamt = Big(this.app.wallet.returnBalance());
      thisamt = thisamt.minus(thisfee);
    }
console.log("ON NEW BLOCK IN SPAMMER 4: ");
    ...
    if (thisamt.gt(0)) {
      ...
      console.log("creating: " + thisamt.toFixed(8) + " -- " + thisfee.toFixed(8) + " === " + this.app.wallet.returnBalance());
