...
Chat.prototype.initialize = function initialize() {
  ...
  for (let p = 0; p < this.chat.rooms.length; p++) {
    ...
    if (cf.publickey != "All") {
      chat_self.app.network.addPeer(cf.host, cf.port, 0, 1, 0); // only txs
    }
  }
...
console.log("New Friend 2: " + JSON.stringify(newfriend));
      ...
      // DNS entry
      if (chat_self.app.crypt.isPublicKey(newfriend.publickey) == 1) {
...
console.log("adding the chat room...");
        chat_self.addChatRoom(chat_self.chat.rooms.length-1);
console.log("added the chat room...");
...
console.log("DNS works? " + JSON.stringify(answer));
